...
### 문제
- 이전 알고리즘(bbox + 비율 정규화 + 리사이즈)은 bbox 검출이 부정확하면 콘텐츠를 잘라낼 가능성이 있었음
- 사용자: "제품을 인식하지 못하고 그냥 잘라버리는데 이방법은 계속해서 문제가 될것같은데"

### 해결: fit-and-letterbox
**핵심 변경**: 비율 매칭을 위해 **콘텐츠를 잘라내는 대신 축소 + 흰배경 패딩**으로 처리.

알고리즘:
1. 콘텐츠 bbox 감지 (실패해도 안전 — 단순 fit 모드로 fallback)
2. bbox + 12% 마진을 **이미지 경계 안에서만** 크롭 (흰 여백만 제거, 콘텐츠 절대 손실 X)
3. 비율 유지로 1500×2250 안에 들어가도록 **축소** (`scale = min(target_w/cw, target_h/ch)`)
4. 흰배경 캔버스에 **중앙 letterbox**

### 보장
- 콘텐츠가 잘리는 일이 수학적으로 불가능 (`scale ≤ 1` if content fits, `scale < 1` if larger)
- bbox 검출이 부정확해도 안전 — 최악의 경우 전체 이미지를 fit
- 안전망: bbox가 너무 좁게 잡히면 (이미지의 30% 미만) 전체 이미지로 폴백

### 검증
- 케이스 5 (제품이 2250×2250 전체 차지) → 출력에서 좌우/하단 빨간 디테일 모두 보존, 위쪽 letterbox
- 22개 단위 테스트 모두 통과

---

## 2026-05-07 (16차) — 크롭 여백 5% → 12% 확대 + 파일명 100_list.jpg

### 변경
- 실제 결과에서 제품이 가장자리에 닿아 잘려 보이는 문제 발견 → `margin_ratio` 0.05 → **0.12** (12%)로 확대
- 결과: 좌우/상하 모두 충분한 흰배경 여유 확보, 어떤 케이스든 제품과 프레임 사이 breathing room 있음
- 크롭 파일명 `main.jpg` → `100_list.jpg`로 변경

### 검증
- 케이스 1~4 (중앙/좌측/우측/매우 넓은/JPEG 노이즈): 모두 좌우 100px에 흰배경 100% 확보
- 22개 단위 테스트 모두 통과

---

## 2026-05-07 (15차) — 스마트 크롭 JPEG 노이즈 강건성 보강

### 문제
실제 출력에서 제품이 여전히 잘려나가는 케이스 발견. 원인: 입력 이미지가 JPEG 압축을 거치면 흰배경(255,255,255)도 픽셀 노이즈로 RGB 채널이 245 미만으로 떨어져 콘텐츠로 오인식됨.
- 검증: 흰배경+검정 사각형 이미지를 JPEG quality 70으로 저장 → 흰배경 인식률 39%만 (61%가 노이즈로 콘텐츠 오인식) → bbox가 거의 전체 이미지로 잡혀서 비율 정규화 효과 무력화

### 해결
- 임계값 245 → **235**로 완화
- 행/열별 콘텐츠 픽셀 비율 ≥ **0.5%** (`content_min_ratio`)일 때만 콘텐츠 행/열로 인정
- 알파 채널 임계값 16 → **32**로 (반투명 안티앨리어싱 무시)

### 결과
- JPEG 노이즈 시나리오: bbox가 실제 콘텐츠 영역(100-2150) 정확 검출
- 가장자리 닿는 시나리오: 양쪽 디테일 보존 유지
- 22개 단위 테스트 모두 통과

### 설정 변경
- `crop_vertical.white_threshold`: 245 → 235
- `crop_vertical.content_min_ratio`: 0.005 신규
- `crop_vertical.margin_ratio`: 0.05 명시화

---

## 2026-05-07 (14차) — 1500×2250 크롭 스마트 알고리즘 (제품 절대 손실 없음)

### 변경 (2회 반복 개선)

**1차 시도: 콘텐츠 중심 윈도우 이동**
- 좌우 375px 단순 크롭 → 제품 중심으로 1500-wide 윈도우 이동
- 한계: 제품이 1500px보다 넓으면 양쪽이 잘려나가는 문제 잔존

**2차 시도 (최종): 흰배경 패딩 + 비율 정규화 + 리사이즈**
- 콘텐츠 bbox 감지 → 5% 여유 패딩 → 2:3 비율로 박스 확장 → 흰배경 캔버스 합성 → 1500×2250 LANCZOS 리사이즈
- 제품이 어떤 크기/위치에 있어도 **콘텐츠 100% 보존** (잘려나감 0)
- 박스가 이미지 경계 밖으로 벗어나도 안전 (캔버스가 흰배경으로 채움)

### 알고리즘 (최종)
1. 콘텐츠 bbox `(x_min, y_min, x_max, y_max)` 감지 (RGB ≥ 245 또는 알파 ≤ 16 = 배경)
2. 콘텐츠 폭/높이의 5%를 여유 패딩으로 추가
3. 2:3 비율 (`target_w/target_h`)로 박스 확장:
   - 박스가 가로 우세 → 세로 늘림
   - 박스가 세로 우세 → 가로 늘림
4. 흰배경 캔버스에 콘텐츠 부분 합성 (이미지 경계 밖은 흰색)
5. 최종 1500×2250로 LANCZOS 리사이즈

### 기술
- `src/exporter/resizer.py` `_do_crop` 재구현 + `_detect_content_bbox` 헬퍼 (numpy)
- `config/settings.yaml` `crop_vertical`: `crop_left/crop_right` 제거, `white_threshold`/`margin_ratio` 추가
- 단위 테스트 8개 추가 (`TestSmartCropVertical` 5개 + `TestSmartCropNoContentLoss` 3개) — 총 22개 통과
