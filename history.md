...
## 2026년 4월 21일 업데이트 (8차)

### 🔧 gui3.py 후속 정리

#### 탭 이름·순서 변경
- "임시 옵션" 탭 → **"메인"** 으로 이름 변경
- 메인 탭을 제일 앞(첫 번째)으로 이동
- 탭 내 안내 문구("이 탭은 테스트용 임시 옵션입니다.") 제거

#### 설정 탭 불필요 섹션 제거
메인 탭(포토룸 통합방식)에서 사용하지 않는 항목 삭제:

| 제거 섹션 | 이유 |
|-----------|------|
| 처리 프로바이더 선택 | 메인 탭이 자체 Vision 선택 보유 |
| remove.bg 옵션 | 메인 탭은 Photoroom만 사용 |
| 누끼 합성 그림자 옵션 | 메인 탭은 Photoroom 그림자 사용 |
| Gemini AI 그림자 프롬프트 | 메인 탭 미사용 |
| Grok AI 그림자 프롬프트 | 메인 탭 미사용 |
| 음성 합성(TTS) | AI 회의 기능 없음 |
| OpenCV 보정 옵션 | 메인 탭은 Claid만 사용 |

4,961줄 → **3,973줄** (988줄 감소)

#### 실행 오류 수정
- `_save_opencv_settings` 메서드 삭제 후 UI 참조 잔류 → OpenCV UI 섹션 및 `_load_settings` 내 opencv 로드 코드 함께 제거

---

### 🆕 조건 탭 — 편집 가능한 라우팅 규칙

기존 읽기 전용 조건 표시를 **완전 편집 가능**한 규칙 카드로 전면 재설계
...
- 각 규칙 카드:
  ...
  - **조건 4개** (드롭다운):
    - 라벨컷: any / 예 / 아니오
    - 촬영각도: any / top_down / front / side
    - 이미지종류: any / full / detail / worn / package
    - 배경: any / clean / colored
  - **처리 체크박스 3개**: 누끼(배경제거) / 그림자 / 보정(Claid)
  - ↑ ↓ 우선순위 이동 버튼, ✕ 삭제 버튼
- 상단 툴바: **+ 조건 추가** / **저장** / **기본값 복원**

#### 기본 규칙 (우선순위 순)
| 우선순위 | 이름 | 조건 | 처리 |
|---|---|---|---|
| 1 | 라벨/바코드컷 | is_label_cut=예 | 없음 |
| 2 | 수직촬영(탑다운) | shooting_angle=top_down | 보정만 |
| 3 | 디테일컷(흰배경) | image_type=detail + background=clean | 누끼+보정 |
| 4 | 디테일컷(유색배경) | image_type=detail + background=colored | 보정만 |
| 5 | 전체컷(기본) | 조건 없음(나머지 전체) | 누끼+그림자+보정 |

#### 저장/로드
- `config/routing_rules.yaml` 에 저장
- 앱 시작 시 자동 로드, 파일 없으면 기본값 사용

#### pipeline.py 연동
- `process_single_unified_photoroom`에 `routing_rules: list = None` 파라미터 추가
- Vision API 분류 후 규칙 순서대로 평가 → 첫 매칭 규칙의 처리 플래그 적용
- 규칙 없거나 매칭 없을 시 기존 하드코딩 로직으로 폴백
