...
class TTSEngine:
    ...
    def __init__(self, provider: str = "off",
        ...
        self._media_tmp_path = None  # os.startfile로 재생 중인 임시 파일 경로
    ...
    def stop(self):
        ...
        # os.startfile로 열린 미디어 플레이어 프로세스 종료
        self._kill_media_players()

    def _kill_media_players(self):
        """os.startfile로 열린 미디어 플레이어를 종료한다."""
        import subprocess
        try:
            # Windows Media Player / Movies & TV 등 기본 플레이어 종료
            for proc_name in ["wmplayer.exe", "Video.UI.exe", "msedge.exe",
        ...
        if self._pyttsx3_engine:
            try:
                ...
                pass
        # 임시 파일 정리
        if self._media_tmp_path:
            import os
            try:
                os.unlink(self._media_tmp_path)
            except Exception:
                pass
            self._media_tmp_path = None
    ...
    def _speak_windows(self, text: str, evaluator: str):
        ...
        try:
            ...
            # stop_flag 감시 스레드: runAndWait 블로킹 중에도 중지 가능
            self._pyttsx3_engine = engine

            def _watch_stop():
                import time
                while self._speaking and not self._stop_flag:
                    time.sleep(0.1)
                if self._stop_flag:
                    try:
                        engine.stop()
