...
class App(tk.Tk):
    ...
    def _build_main_tab(self):
        ...
        self.btn_run_single = ttk.Button(action_bar, text="  단일 처리  ",
            command=lambda: self._run("single"), style="Accent.TButton")
        self.btn_run_single.pack(side="left", padx=(0, 6), ipady=3)
        self.btn_run_batch = ttk.Button(action_bar, text="  배치 처리  ",
        ...
        self.btn_stop.pack(side="left", padx=(0, 10), ipady=3)
    ...
    def _build_prompt_tab(self):
        ...
        self.txt_user = scrolledtext.ScrolledText(usr_lf, height=14,
            ...
            borderwidth=1)
        self.txt_user.pack(fill="both", expand=True, padx=2, pady=(0, 2))
    ...
    def _build_settings_tab(self):
        ...
        ttk.Label(gs_top, text="생성 순서:", style="Card.TLabel").pack(side="left", padx=(2, 6))
        ...
        gs_f = ttk.Frame(gs_lf, style="Card.TFrame")
        gs_f.pack(fill="x")
        ...
        for r, (label, attr, height, tooltip, default) in enumerate(prompt_items):
            lbl = ttk.Label(gs_f, text=label, style="Card.TLabel")
        ...
        # ══════════════════════════════════════
        #  10. 회의 효과 (애니메이션 / 영상 녹화)
        # ══════════════════════════════════════
        fx_lf = tk.LabelFrame(sf, text=" 회의 효과 (애니메이션 / 영상) ",
                              font=(FONT_FAMILY, 11, "bold"),
                              bg=CARD_BG, fg="#374151", padx=10, pady=6)
        fx_lf.pack(fill="x", pady=(0, 8))

        fx_f = ttk.Frame(fx_lf, style="Card.TFrame")
        fx_f.pack(fill="x")

        # 타이핑 애니메이션
        self.var_typing_anim = tk.BooleanVar(value=True)
        r = 0
        lbl = ttk.Checkbutton(fx_f, text="타이핑 애니메이션", variable=self.var_typing_anim)
        lbl.grid(row=r, column=0, sticky="w", padx=(2, 8), pady=4)
        ToolTip(lbl, "회의 채팅창에서 텍스트가 한 글자씩 타이핑되는 효과.\n활성화하면 실제 사람이 입력하는 느낌을 줌")
        ttk.Label(fx_f, text="속도:", style="Card.TLabel").grid(row=r, column=1, padx=(8, 4), pady=4)
        self.var_typing_speed = tk.StringVar(value="30")
        ttk.Entry(fx_f, textvariable=self.var_typing_speed, width=5,
                  font=(FONT_FAMILY, 10)).grid(row=r, column=2, sticky="w", pady=4)
        ttk.Label(fx_f, text="ms/글자", style="Card.TLabel",
                  font=(FONT_FAMILY, 9)).grid(row=r, column=3, sticky="w", padx=4, pady=4)

        # 영상 녹화
        r += 1
        self.var_video_export = tk.BooleanVar(value=False)
        lbl2 = ttk.Checkbutton(fx_f, text="회의 영상 자동 저장 (MP4)", variable=self.var_video_export)
        lbl2.grid(row=r, column=0, sticky="w", padx=(2, 8), pady=4)
        ToolTip(lbl2, "회의 종료 후 대화 내용을 MP4 영상으로 자동 저장.\nMoviePy + Pillow 기반 (무료).\n출력 폴더에 deliberation_YYYYMMDD_HHMMSS.mp4로 저장")

        vid_f = ttk.Frame(fx_f, style="Card.TFrame")
        vid_f.grid(row=r, column=1, columnspan=3, sticky="w", pady=4)
        ttk.Label(vid_f, text="해상도:", style="Card.TLabel",
                  font=(FONT_FAMILY, 9)).pack(side="left", padx=(0, 4))
        self.var_video_res = tk.StringVar(value="1280x720")
        ttk.Combobox(vid_f, textvariable=self.var_video_res,
                     values=["1280x720", "1920x1080", "960x540"],
                     width=11, state="readonly", font=(FONT_FAMILY, 9)).pack(side="left", padx=(0, 8))
        ttk.Label(vid_f, text="FPS:", style="Card.TLabel",
                  font=(FONT_FAMILY, 9)).pack(side="left", padx=(0, 4))
        self.var_video_fps = tk.StringVar(value="24")
        ttk.Entry(vid_f, textvariable=self.var_video_fps, width=4,
                  font=(FONT_FAMILY, 9)).pack(side="left")

        fx_btn_f = ttk.Frame(fx_lf, style="Card.TFrame")
        fx_btn_f.pack(fill="x", pady=(4, 0))
        ttk.Button(fx_btn_f, text="설정 저장", command=self._save_delib_fx_settings,
                   style="Accent.TButton").pack(side="left", padx=(2, 10))
        self.fx_status = ttk.Label(fx_btn_f, text="", style="Card.TLabel")
        self.fx_status.pack(side="left")

        # ══════════════════════════════════════
        #  11. 카테고리별 여백
        # ══════════════════════════════════════
        cat_lf = tk.LabelFrame(sf, text=" 카테고리별 여백 규칙 (더블클릭으로 편집) ",
                               font=(FONT_FAMILY, 11, "bold"),
                               bg=CARD_BG, fg="#374151", padx=10, pady=6)
    ...
    def _load_settings(self):
        try:
            ...
            # 회의 효과 설정 로드
            fx = data.get("deliberation_effects", {})
            self.var_typing_anim.set(fx.get("typing_animation", True))
            self.var_typing_speed.set(str(fx.get("typing_speed", 30)))
            self.var_video_export.set(fx.get("video_export", False))
            vw = fx.get("video_width", 1280)
            vh = fx.get("video_height", 720)
            self.var_video_res.set(f"{vw}x{vh}")
            self.var_video_fps.set(str(fx.get("video_fps", 24)))
    ...
    def _save_delib_fx_settings(self):
        """회의 효과 설정 저장."""
        try:
            data = load_yaml(SETTINGS_PATH)
            fx = data.setdefault("deliberation_effects", {})
            fx["typing_animation"] = self.var_typing_anim.get()
            fx["typing_speed"] = int(self.var_typing_speed.get())
            fx["video_export"] = self.var_video_export.get()
            res = self.var_video_res.get().split("x")
            fx["video_width"] = int(res[0])
            fx["video_height"] = int(res[1])
            fx["video_fps"] = int(self.var_video_fps.get())
            save_yaml(SETTINGS_PATH, data)
            self.fx_status.config(
                text=f"저장 완료 ({datetime.now().strftime('%H:%M:%S')})",
                foreground=SUCCESS)
        except Exception as e:
            self.fx_status.config(text=f"저장 실패: {e}", foreground=DANGER)
    ...
    def _open_deliberation_window(self):
        ...
        self._delib_recording = []  # 영상 녹화용 프레임 데이터
    ...
    def _is_typing_enabled(self):
        """타이핑 애니메이션 활성화 여부."""
        try:
            return hasattr(self, 'var_typing_anim') and self.var_typing_anim.get()
        except Exception:
            return False

    def _type_text(self, chat, text, tags, callback=None, idx=0):
        """한 글자씩 타이핑 효과로 텍스트 삽입."""
        if idx < len(text):
            chat.insert("end", text[idx], tags)
            self._delib_auto_scroll()
            speed = int(self.var_typing_speed.get()) if hasattr(self, 'var_typing_speed') else 30
            self.after(speed, self._type_text, chat, text, tags, callback, idx + 1)
        else:
            if callback:
                callback()

    def _record_delib_frame(self, provider, name, text):
        """회의 녹화용 프레임 데이터 수집."""
        if not hasattr(self, '_delib_recording'):
            self._delib_recording = []
        if not hasattr(self, 'var_video_export') or not self.var_video_export.get():
            return
        self._delib_recording.append({
            "provider": provider,
            "name": name,
            "text": text,
            "timestamp": datetime.now().strftime("%H:%M:%S"),
            "phase": getattr(self, '_delib_current_phase', 0),
        })
    ...
    def _on_deliberation_inner(self, chat, phase, provider, data):
        ...
        if phase == "mc":
            ...
            chat.insert("end", f"  ({ts})\n", "system_tag")
            # 타이핑 애니메이션
            if self._is_typing_enabled():
                self._type_text(chat, f"  {speech}\n", ("mc_text", "mc_bg"))
            else:
                chat.insert("end", f"  {speech}\n", ("mc_text", "mc_bg"))
            # TTS 음성 출력 (사회자)
            self._tts_speak(speech, "mc")
            # 회의 녹화 데이터 수집
            self._record_delib_frame("mc", "사회자", speech)
            self._delib_auto_scroll()
            return
        ...
        if phase == "phase" and isinstance(data, dict):
            ...
            if data.get("deep_explore"):
                chat.insert("end",
                            "\n  ** 심화 탐색 모드 — 새로운 접근법 필요 **\n",
                            "deep_tag")
            self._record_delib_frame("mc", "진행", f"[{p}단계] {title}")
            self._delib_auto_scroll()
            return
        ...
        if speech:
            ...
            # 회의 녹화 데이터 수집
            self._record_delib_frame(provider, name, speech)
