...
class ImageEditPipeline:
    ...
    def evaluate_result(self, original_path: str, result_path: str,
        ...
        p1_results = self._call_all_vision_apis(
            images, pb.build_phase1_system(),
            pb.build_phase1_prompt(current_params), on_log)
        ...
        p2_results = self._call_all_vision_apis(
            images, pb.build_phase2_system(),
            pb.build_phase2_prompt(all_p1_text), on_log)
        ...
        p3_result = self._call_single_vision_api(
            ...
            pb.build_phase3_system(), pb.build_phase3_prompt(all_so_far))

        if p3_result.get("error"):
            # fallback: 첫 번째 성공 API로 재시도
        ...
        _delib("phase", "system", {
            ...
            "deep_explore": iteration_count >= 5})

        problem_text = _json.dumps(p3_result, indent=2, ensure_ascii=False)
        p4_results = self._call_all_vision_apis(
            images, pb.build_phase4_system(),
            pb.build_phase4_prompt(problem_text, iteration_count), on_log)
        ...
        for fix_round in range(1, max_fix_rounds + 1):
            ...
            p5_results = self._call_all_vision_apis(
                images, pb.build_phase5_system(),
                pb.build_phase5_prompt(solutions_text, fix_round), on_log)
        ...
        p6_result = self._call_single_vision_api(
            self._vision_provider, images,
            pb.build_phase6_system(),
            pb.build_phase6_prompt(final_text, total_rounds))
