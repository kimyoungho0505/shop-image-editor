...
class ImageEditPipeline:
    ...
    def process_single(self, image_path: str, category: str, output_dir: str,
        ...
        # 의류 착용샷(worn)이고 AI가 그림자 불필요로 판단한 경우
        # → OpenCV fallback을 무시하고 AI 판단을 존중
        # (마네킹/행거 의류는 바닥이 안 보여 그림자 불필요,
        #  단 발/신발이 보이는 착용샷은 AI가 shadow_needed=true로 판단)
        _is_clothing_no_shadow = (
            not needs_shadow
            and image_type in ("worn",)
        )

        # ★ 마네킹 감지 + 생성형 그림자 → 강제 활성화
        # Gemini가 마네킹 제거 + 그림자 생성을 동시에 처리하므로
        # 의류 no-shadow 예외보다 마네킹 제거가 우선
        _mannequin_force = (
            instruction.has_mannequin
            and _is_generative_shadow
        )

        if _mannequin_force and not needs_shadow:
            _log(f"  마네킹 감지 + 생성형 그림자({self._shadow_provider}) → 마네킹 제거를 위해 강제 활성화")
            needs_shadow = True
        elif needs_shadow and not has_real_shadow and not _is_generative_shadow:
            _log(f"  ⚠ AI가 그림자 있다고 판단했으나 원본 바닥에 그림자 감지 안 됨 → 그림자 처리 스킵", "warn")
            needs_shadow = False
        elif not needs_shadow and has_real_shadow and not _is_clothing_no_shadow:
            _log(f"  ⚠ AI가 그림자 없다고 판단했으나 원본 바닥에 그림자 감지됨 → 그림자 처리 진행", "warn")
            needs_shadow = True
        elif _is_clothing_no_shadow and has_real_shadow:
            _log(f"  ℹ 의류 착용샷 — AI 판단(그림자 불필요) 존중, OpenCV 감지 무시", "info")
            ...
            if PhotoroomClient.should_process(image_type, background):
                ...
                if bg_result:
                    ...
                    if pre_cropped:
                        ...
                        edit_actions.append(f"{self._bg_provider}: 배경제거 (크롭 완료)")
                    else:
                        output_w = self._settings.get("output", {}).get("width", 1000)
    ...
    def _gemini_add_shadow(self, image_bytes: bytes,
        ...
        try:
            ...
            mannequin_text = ""
            if has_mannequin:
                mannequin_text = gs_cfg.get("mannequin_prompt",
                    "제품 하단에 보이는 마네킹/토르소/스탠드 잔여물을 완전히 제거하세요. "
                    "잘린 부분은 원래 의류의 자연스러운 마감선으로 복원하세요. "
                    "마네킹이 있던 자리를 깨끗한 흰색 배경으로 채우세요. ")
                _log(f"  마네킹 잔여물 제거 프롬프트 포함")

            contents = []

            if has_mannequin and original_bytes:
                # ★ 마네킹 모드: 원본 이미지를 메인으로 전송
                # remove.bg 누끼에 남은 마네킹 잔여물을 우회하기 위해
                # Gemini가 원본에서 직접 배경제거 + 마네킹 제거 + 그림자 생성을 한 번에 처리
                contents.append(types.Part.from_bytes(
                    data=original_bytes, mime_type=_detect_mime(original_bytes)))

                # 마네킹 모드 전용 프롬프트 (원본 이미지 기반)
                mannequin_full_prompt = gs_cfg.get("mannequin_full_prompt",
                    ...
                    "럭셔리 이커머스 제품 사진처럼 고급스럽게 만들어주세요.")
                contents.append(mannequin_full_prompt)
