...
class ImageEditPipeline:
    ...
    def evaluate_result(self, original_path: str, result_path: str,
                        ...
                        max_discussion_rounds: int = 10) -> dict:
        """3-API 합의 시스템으로 원본 vs 결과를 비교 평가한다.

        Flow:
          1) Claude, ChatGPT, Gemini 3개 동시 독립 평가
          2) 결과 합쳐서 다시 3개에게 토론 라운드 (최대 max_discussion_rounds)
          3) 합의 도출 → 최종 verdict + adjusted_params 반환

        Args:
            on_deliberation: 토론 내용 실시간 콜백 (provider, round, message_dict)
            max_discussion_rounds: 최대 토론 라운드 수
        ...
        _log = on_log or (lambda msg, tag="info": logger.info(msg))
        _delib = on_deliberation or (lambda provider, round_num, data: None)
        ...
        # ── 1단계: 3개 API 독립 평가 ──
        _log(f"  [자동수정] ━━ 3-AI 합의 평가 시작 ━━")
        sys_prompt = self._prompt_builder.build_deliberation_system_prompt()
        user_prompt = self._prompt_builder.build_deliberation_initial_prompt(current_params)

        initial_results = self._call_all_vision_apis(images, sys_prompt, user_prompt, on_log)

        # 성공한 평가만 필터
        valid_results = [r for r in initial_results if not r.get("error")]
        if not valid_results:
            _log(f"  [자동수정] 모든 API 평가 실패, 기본값 반환", "error")
            return {
                ...
                "code_issues": [], "deliberation_log": initial_results,
            }

        # 실시간 콜백: 초기 평가 결과
        for r in valid_results:
            _delib(r.get("evaluator", "?"), 0, r)

        # API가 1개만 성공하면 토론 없이 바로 반환
        if len(valid_results) == 1:
            _log(f"  [토론] API 1개만 성공, 토론 생략")
            return self._build_final_result(valid_results[0], current_params,
                                            deliberation_log=initial_results)

        # ── 2단계: 토론 라운드 ──
        disc_sys = self._prompt_builder.build_discussion_system_prompt()
        latest_results = list(valid_results)
        deliberation_log = [{"round": 0, "type": "initial", "results": initial_results}]

        for round_num in range(1, max_discussion_rounds + 1):
            # 합의 여부 확인
            consensus_count = sum(
                1 for r in latest_results
                if r.get("consensus_reached", False)
            )
            if consensus_count >= len(latest_results) and round_num > 1:
                _log(f"  [토론] {round_num - 1}라운드에서 합의 도달!")
                break

            # 모든 평가 결과를 텍스트로 합침
            all_evals_text = _json.dumps(latest_results, indent=2, ensure_ascii=False)
            disc_prompt = self._prompt_builder.build_discussion_prompt(
                all_evals_text, round_num, max_discussion_rounds
            )

            _log(f"  [토론] {round_num}/{max_discussion_rounds} 라운드 진행 중...")
            round_results = self._call_all_vision_apis(
                images, disc_sys, disc_prompt, on_log
            )

            valid_round = [r for r in round_results if not r.get("error")]
            deliberation_log.append({
                "round": round_num, "type": "discussion", "results": round_results
            })

            # 실시간 콜백
            for r in valid_round:
                _delib(r.get("evaluator", "?"), round_num, r)

            if valid_round:
                latest_results = valid_round

            # 합의 확인
            consensus_count = sum(
                1 for r in valid_round if r.get("consensus_reached", False)
            )
            if consensus_count >= len(valid_round):
                _log(f"  [토론] {round_num}라운드에서 합의 도달!")
                break

        # ── 3단계: 최종 합의 도출 ──
        _log(f"  [토론] 최종 합의 도출 중...")
        cons_sys = self._prompt_builder.build_consensus_system_prompt()
        final_evals_text = _json.dumps(latest_results, indent=2, ensure_ascii=False)
        cons_prompt = self._prompt_builder.build_consensus_prompt(
            final_evals_text, len(deliberation_log) - 1
        )

        # 합의 도출은 사용자가 지정한 메인 프로바이더 사용
        consensus_data = self._call_single_vision_api(
            self._vision_provider, images, cons_sys, cons_prompt
        )

        if consensus_data.get("error"):
            _log(f"  [토론] 합의 도출 실패, 토론 결과 평균 사용", "warn")
            consensus_data = self._merge_results_fallback(latest_results)

        _delib("consensus", -1, consensus_data)
        deliberation_log.append({
            "round": "final", "type": "consensus", "result": consensus_data
        })

        result = self._build_final_result(consensus_data, current_params,
                                          deliberation_log=deliberation_log)

        _log(f"  [자동수정] 합의 결과: {result['verdict']} (평균 {result['average_score']:.1f}/10)")
        if result.get("code_issues"):
            _log(f"  [자동수정] 코드 수정 제안 {len(result['code_issues'])}건:")
    ...
    def process_with_refinement(self, image_path: str, category: str,
                                ...
                                on_deliberation: Callable = None) -> dict:
