...
class App(TkinterDnD.Tk if _HAS_DND else tk.Tk):
    def __init__(self):
        ...
        self._vf_file_stages = {}  # fname -> {"stages": {stage: "done"|"active"|"skip"}, "status": "pending"|"processing"|"done"|"fail"}
        self._vf_dlg = None
        self._load_state()
        self._build_ui()
        self._load_configs()
    ...
    def _save_state(self):
        ...
        self._state["skip_analysis"] = self.var_skip_analysis.get()
        self._state["pre_cropped"] = self.var_pre_cropped.get()
        self._state["auto_refine"] = self.var_auto_refine.get()
        self._state["max_iterations"] = self.var_max_iterations.get()
        self._state["concurrent_workers"] = self.var_workers.get()
        self._state["vision_provider"] = self.var_vision_provider.get()
        self._state["bg_provider"] = self.var_bg_provider.get()
        self._state["enhance_provider"] = self.var_enhance_provider.get()
        self._state["shadow_provider"] = self.var_shadow_provider.get()
        self._state["shadow_judge_mode"] = self.var_shadow_judge.get()
        self._state["shadow_composite"] = self.var_shadow_composite.get()
        try:
    ...
    def _build_ui(self):
        ...
        self.notebook.add(self.tab_main, text="  실행  ")
        self._build_main_tab()

        self.tab_prompt = ttk.Frame(self.notebook)
        self.notebook.add(self.tab_prompt, text="  프롬프트 편집  ")
        self._build_prompt_tab()

        self.tab_eval = ttk.Frame(self.notebook)
        self.notebook.add(self.tab_eval, text="  평가 프롬프트  ")
        self._build_eval_tab()

        self.tab_shadow_hints = ttk.Frame(self.notebook)
        self.notebook.add(self.tab_shadow_hints, text="  그림자 프롬프트  ")
        self._build_shadow_hints_tab()
    ...
    def _build_main_tab(self):
        ...
        top_area = ttk.Frame(parent)
        top_area.pack(fill="x", padx=12, pady=(8, 0))

        # ── 1. 폴더 섹션 (좌측 라벨 + 우측 입력) ──
        folder_card = ttk.Frame(top_area, style="Card.TFrame")
        ...
        # ── 2. 프로바이더 + 옵션 (좌우 분할) ──
        mid_row = ttk.Frame(top_area)
        mid_row.pack(fill="x", pady=(0, 6))
        mid_row.columnconfigure(0, weight=3)
        mid_row.columnconfigure(1, weight=1)

        # 프로바이더 카드 (좌측, 넓게)
        prov_card = tk.LabelFrame(mid_row, text=" 프로바이더 ", font=(FONT_FAMILY, 9, "bold"),
                                  bg=CARD_BG, fg="#555", padx=8, pady=4)
        prov_card.grid(row=0, column=0, sticky="nsew", padx=(0, 4))

        prov_grid = ttk.Frame(prov_card, style="Card.TFrame")
        prov_grid.pack(fill="x")

        # 분석 / 배경 / 보정 (1행)
        c = 0
        ttk.Label(prov_grid, text="분석", style="Card.TLabel",
                  font=(FONT_FAMILY, 9, "bold")).grid(row=0, column=c, sticky="w", padx=(0, 4))
        c += 1
        for txt, val in [("Claude", "claude"), ("ChatGPT", "chatgpt"), ("Gemini", "gemini"), ("Grok", "grok")]:
            ttk.Radiobutton(prov_grid, text=txt, variable=self.var_vision_provider,
                            value=val).grid(row=0, column=c, padx=2)
            c += 1

        sep1 = ttk.Separator(prov_grid, orient="vertical")
        sep1.grid(row=0, column=c, rowspan=1, sticky="ns", padx=6)
        c += 1

        ttk.Label(prov_grid, text="배경", style="Card.TLabel",
                  font=(FONT_FAMILY, 9, "bold")).grid(row=0, column=c, sticky="w", padx=(0, 4))
        c += 1
        for txt, val in [("Photoroom", "photoroom"), ("remove.bg", "removebg"), ("복합", "hybrid")]:
            ttk.Radiobutton(prov_grid, text=txt, variable=self.var_bg_provider,
                            value=val).grid(row=0, column=c, padx=2)
            c += 1

        sep2 = ttk.Separator(prov_grid, orient="vertical")
        sep2.grid(row=0, column=c, rowspan=1, sticky="ns", padx=6)
        c += 1

        ttk.Label(prov_grid, text="보정", style="Card.TLabel",
                  font=(FONT_FAMILY, 9, "bold")).grid(row=0, column=c, sticky="w", padx=(0, 4))
        c += 1
        for txt, val in [("Claid.ai", "claid"), ("OpenCV", "opencv")]:
            ttk.Radiobutton(prov_grid, text=txt, variable=self.var_enhance_provider,
                            value=val).grid(row=0, column=c, padx=2)
            c += 1

        # 그림자 (2행)
        shadow_f = ttk.Frame(prov_card, style="Card.TFrame")
        shadow_f.pack(fill="x", pady=(4, 0))
        ttk.Label(shadow_f, text="그림자", style="Card.TLabel",
        ...
        for txt, val in [("API", "api_shadow"), ("Gemini", "gemini_shadow"),
                         ...
                         ("SAM-CPU", "sam_cpu")]:
            ttk.Radiobutton(shadow_f, text=txt, variable=self.var_shadow_provider,
                            value=val).pack(side="left", padx=2)
        self.rb_sam_gpu_b_main = ttk.Radiobutton(shadow_f, text="GPU-B",
                        variable=self.var_shadow_provider, value="sam_gpu_b")
        self.rb_sam_gpu_b_main.pack(side="left", padx=2)
        self.rb_sam_gpu_l_main = ttk.Radiobutton(shadow_f, text="GPU-L",
                        variable=self.var_shadow_provider, value="sam_gpu_l")
        self.rb_sam_gpu_l_main.pack(side="left", padx=2)
        self.rb_sam_gpu_h_main = ttk.Radiobutton(shadow_f, text="GPU-H",
                        variable=self.var_shadow_provider, value="sam_gpu_h")
        self.rb_sam_gpu_h_main.pack(side="left", padx=2)
        ttk.Radiobutton(shadow_f, text="없음", variable=self.var_shadow_provider,
                        value="none").pack(side="left", padx=2)

        # 그림자 판단 모드
        judge_f = ttk.Frame(prov_card, style="Card.TFrame")
        judge_f.pack(fill="x", pady=(4, 0))
        self.var_shadow_judge = tk.StringVar(value="auto")
        lbl_judge = ttk.Label(judge_f, text="판단모드", style="Card.TLabel",
                  font=(FONT_FAMILY, 9, "bold"))
        lbl_judge.pack(side="left", padx=(0, 6))
        ToolTip(lbl_judge,
                "AI 자동: Vision API가 촬영 상황 분석 후 그림자 필요 여부 자동 결정\n"
                "항상 생성: 모든 이미지에 그림자 생성\n"
                "항상 스킵: 그림자 생성 안 함")
        for txt, val in [("AI 자동", "auto"), ("항상 생성", "always"), ("항상 스킵", "never")]:
            ttk.Radiobutton(judge_f, text=txt, variable=self.var_shadow_judge,
                            value=val).pack(side="left", padx=2)

        # 그림자 합성 방식 (3행)
        composite_f = ttk.Frame(prov_card, style="Card.TFrame")
        composite_f.pack(fill="x", pady=(4, 0))
        self.var_shadow_composite = tk.StringVar(
            value=self._state.get("shadow_composite", "overlay"))
        lbl_comp = ttk.Label(composite_f, text="합성방식", style="Card.TLabel",
                  font=(FONT_FAMILY, 9, "bold"))
        lbl_comp.pack(side="left", padx=(0, 6))
        ToolTip(lbl_comp,
                "기존(오버레이): AI 결과 위에 원본 누끼를 덮어씌움\n"
                "레이어분리: AI 결과에서 그림자만 추출하여 합성\n"
                "→ 제품 변형 문제가 발생하면 레이어분리 사용 권장")
        ttk.Radiobutton(composite_f, text="기존(오버레이)",
                        variable=self.var_shadow_composite,
                        value="overlay").pack(side="left", padx=2)
        ttk.Radiobutton(composite_f, text="레이어분리",
                        variable=self.var_shadow_composite,
                        value="layer_extract").pack(side="left", padx=2)

        # 옵션 카드 (우측, 좁게)
        opt_card = tk.LabelFrame(mid_row, text=" 옵션 ", font=(FONT_FAMILY, 9, "bold"),
                                 bg=CARD_BG, fg="#555", padx=8, pady=4)
        opt_card.grid(row=0, column=1, sticky="nsew", padx=(4, 0))

        self.var_skip_bg = tk.BooleanVar(value=self._state.get("skip_bg", False))
        self.var_skip_analysis = tk.BooleanVar(value=self._state.get("skip_analysis", False))
        self.var_pre_cropped = tk.BooleanVar(value=self._state.get("pre_cropped", False))
        self.var_auto_refine = tk.BooleanVar(value=self._state.get("auto_refine", False))
        self.var_max_iterations = tk.IntVar(value=self._state.get("max_iterations", 3))

        ttk.Checkbutton(opt_card, text="배경 제거 생략",
                        variable=self.var_skip_bg).pack(anchor="w", pady=1)
        ttk.Checkbutton(opt_card, text="AI 분석 생략",
                        variable=self.var_skip_analysis).pack(anchor="w", pady=1)
        cb_crop = ttk.Checkbutton(opt_card, text="크롭 완료 이미지",
                        variable=self.var_pre_cropped)
        cb_crop.pack(anchor="w", pady=1)
        ToolTip(cb_crop, "이미 크롭된 이미지를 입력할 때 사용.\n"
                "크롭/여백/중앙정렬을 건너뛰고\n"
                "누끼 + 보정 + 그림자만 수행합니다.\n"
                "Vision API 참고 이미지도 1장만 사용하여 비용 절감")

        refine_f = ttk.Frame(opt_card, style="Card.TFrame")
        refine_f.pack(anchor="w", pady=1)
        ttk.Checkbutton(refine_f, text="자동 수정",
                        variable=self.var_auto_refine).pack(side="left")
        ttk.Spinbox(refine_f, from_=1, to=10, textvariable=self.var_max_iterations,
                    width=3, font=(FONT_FAMILY, 9)).pack(side="left", padx=(4, 0))
        ttk.Label(refine_f, text="회", style="Card.TLabel",
                  font=(FONT_FAMILY, 9)).pack(side="left", padx=(2, 0))

        # ── 3. 실행 버튼 + 프로그레스 (한 줄) ──
        action_bar = ttk.Frame(top_area)
        action_bar.pack(fill="x", pady=(0, 6))

        self.btn_run_single = ttk.Button(action_bar, text="  파일  ",
            command=lambda: self._run("single"), style="Accent.TButton")
        self.btn_run_single.pack(side="left", padx=(0, 6), ipady=3)
        self.btn_run_batch = ttk.Button(action_bar, text="  폴더  ",
            command=lambda: self._run("batch"), style="Accent.TButton")
        self.btn_run_batch.pack(side="left", padx=(0, 6), ipady=3)
        self.btn_analyze = ttk.Button(action_bar, text="분석만",
            command=lambda: self._run("analyze"))
        self.btn_analyze.pack(side="left", padx=(0, 6), ipady=3)
        self.btn_stop = ttk.Button(action_bar, text="중지", command=self._stop, state="disabled")
        self.btn_stop.pack(side="left", padx=(0, 6), ipady=3)
        ttk.Button(action_bar, text="재시작", command=self._restart,
                   style="Restart.TButton").pack(side="left", padx=(0, 6), ipady=3)
        self.btn_viewfinder = ttk.Button(action_bar, text="뷰파인더",
            command=self._open_viewfinder, state="disabled", style="Viewfinder.TButton")
        self.btn_viewfinder.pack(side="left", padx=(0, 6), ipady=3)

        # 동시 처리 수
        ttk.Label(action_bar, text="동시:", font=(FONT_FAMILY, 9)).pack(side="left", padx=(4, 2))
        self.var_workers = tk.IntVar(value=self._state.get("concurrent_workers", 1))
        cb_workers = ttk.Combobox(action_bar, textvariable=self.var_workers, width=2,
            values=[1, 2, 4, 8], state="readonly", font=(FONT_FAMILY, 9))
        cb_workers.pack(side="left", padx=(0, 4))
        cb_workers.bind("<<ComboboxSelected>>", lambda e: self._save_workers_setting())
        ToolTip(cb_workers, "동시 처리 수\n1 = 순차 처리\n4 = 4개 파이프라인 동시 실행\n유료 API 기준 2~4 권장")

        # 프로그레스 (버튼 우측에 배치)
        self.var_progress = tk.DoubleVar(value=0)
        self.progress = ttk.Progressbar(action_bar, mode="determinate",
                                        variable=self.var_progress, maximum=100)
        self.progress.pack(side="left", fill="x", expand=True, padx=(4, 0))
        self.lbl_progress = ttk.Label(action_bar, text="0%", width=5, anchor="e",
                                      font=(FONT_FAMILY, 9))
    ...
    def _build_prompt_tab(self):
        tab_parent = self.tab_prompt

        # 상단 안내 + 버튼 (스크롤 밖, 고정)
        info_f = ttk.Frame(tab_parent)
        info_f.pack(fill="x", padx=12, pady=(8, 4))
        ttk.Label(info_f, text="AI 비전 분석 프롬프트",
                  style="Header.TLabel").pack(side="left")
        hint = ttk.Label(info_f, text="한글/영어 모두 사용 가능  |  수정 후 [저장] 클릭",
                         font=(FONT_FAMILY, 9), foreground="#6b7280")
        hint.pack(side="left", padx=(12, 0))

        bf = ttk.Frame(tab_parent)
        bf.pack(fill="x", padx=12, pady=(0, 6))
        ttk.Button(bf, text="  저장  ", command=self._save_prompts,
                   style="Accent.TButton").pack(side="left", padx=(0, 8), ipady=2)
        ttk.Button(bf, text="한글 기본값 복원", command=self._reset_prompts).pack(
            side="left", padx=(0, 8), ipady=2)
        ttk.Button(bf, text="파일에서 다시 불러오기", command=self._load_prompts).pack(
            side="left", ipady=2)
        self.prompt_status = ttk.Label(bf, text="", style="TLabel")
        self.prompt_status.pack(side="left", padx=15)

        # 스크롤 가능 영역
        prompt_canvas = tk.Canvas(tab_parent, bg=BG_COLOR, highlightthickness=0)
        prompt_scrollbar = ttk.Scrollbar(tab_parent, orient="vertical", command=prompt_canvas.yview)
        prompt_scroll_frame = ttk.Frame(prompt_canvas)
        prompt_scroll_frame.bind("<Configure>",
            lambda e: prompt_canvas.configure(scrollregion=prompt_canvas.bbox("all")))
        prompt_canvas.create_window((0, 0), window=prompt_scroll_frame, anchor="nw")
        ...
        # 마우스 휠 스크롤
        def _on_prompt_mousewheel(e):
            prompt_canvas.yview_scroll(int(-1 * (e.delta / 120)), "units")
        prompt_canvas.bind("<Enter>", lambda e: prompt_canvas.bind_all("<MouseWheel>", _on_prompt_mousewheel))
        prompt_canvas.bind("<Leave>", lambda e: prompt_canvas.unbind_all("<MouseWheel>"))

        # inner frame 너비를 canvas에 맞추기
        def _sync_prompt_width(e):
            prompt_canvas.itemconfig(prompt_canvas.find_all()[0], width=e.width)
        prompt_canvas.bind("<Configure>", _sync_prompt_width)

        parent = prompt_scroll_frame

        # ── 시스템 프롬프트 ──
        sys_lf = tk.LabelFrame(parent, text=" 시스템 프롬프트 (AI 역할 정의) ",
                               font=(FONT_FAMILY, 10, "bold"),
                               bg=CARD_BG, fg="#555", padx=6, pady=4)
        sys_lf.pack(fill="x", padx=12, pady=(0, 4))

        # 툴팁 아이콘 + 설명
        sys_info = ttk.Label(sys_lf, text="(?) 이 프롬프트의 역할",
                             font=(FONT_FAMILY, 9), foreground=ACCENT, cursor="hand2")
        sys_info.pack(anchor="w", padx=2, pady=(0, 2))
        ToolTip(sys_info,
        ...
        self.txt_system = scrolledtext.ScrolledText(sys_lf, height=7,
            font=(FONT_FAMILY, 10), bg="#fefce8", wrap="word", relief="flat",
            borderwidth=1)
        self.txt_system.pack(fill="x", padx=2, pady=(0, 2))

        # ── 유저 프롬프트 ──
        usr_lf = tk.LabelFrame(parent, text=" 분석 요청 프롬프트 (이미지별 전송) ",
                               font=(FONT_FAMILY, 10, "bold"),
                               bg=CARD_BG, fg="#555", padx=6, pady=4)
        usr_lf.pack(fill="x", padx=12, pady=(0, 8))

        usr_info = ttk.Label(usr_lf, text="(?) 이 프롬프트의 역할",
                             font=(FONT_FAMILY, 9), foreground=ACCENT, cursor="hand2")
        usr_info.pack(anchor="w", padx=2, pady=(0, 2))
        ToolTip(usr_info,
        ...
        self.txt_user = scrolledtext.ScrolledText(usr_lf, height=14,
            font=(FONT_FAMILY, 10), bg="#f0fdf4", wrap="word", relief="flat",
            borderwidth=1)
        self.txt_user.pack(fill="x", padx=2, pady=(0, 2))

    # ── 탭 2.5: 평가 프롬프트 ──
    def _build_eval_tab(self):
        tab_parent = self.tab_eval

        # 상단 안내 + 버튼 (스크롤 밖, 고정)
        info_f = ttk.Frame(tab_parent)
        info_f.pack(fill="x", padx=12, pady=(8, 4))
        ttk.Label(info_f, text="평가 프롬프트",
                  style="Header.TLabel").pack(side="left")
        hint = ttk.Label(info_f, text="개별 검증(PASS/FAIL) + 독립평가(점수제)  |  수정 후 [저장] 클릭",
                         font=(FONT_FAMILY, 9), foreground="#6b7280")
        hint.pack(side="left", padx=(12, 0))

        bf = ttk.Frame(tab_parent)
        bf.pack(fill="x", padx=12, pady=(0, 6))
        ttk.Button(bf, text="  저장  ", command=self._save_eval_prompts,
                   style="Accent.TButton").pack(side="left", padx=(0, 8), ipady=2)
        ttk.Button(bf, text="기본값 복원", command=self._reset_eval_prompts).pack(
            side="left", padx=(0, 8), ipady=2)
        ttk.Button(bf, text="파일에서 다시 불러오기", command=self._load_eval_prompts).pack(
            side="left", ipady=2)
        self.eval_prompt_status = ttk.Label(bf, text="", style="TLabel")
        self.eval_prompt_status.pack(side="left", padx=15)

        # 스크롤 가능 영역
        eval_canvas = tk.Canvas(tab_parent, bg=BG_COLOR, highlightthickness=0)
        eval_scrollbar = ttk.Scrollbar(tab_parent, orient="vertical", command=eval_canvas.yview)
        eval_scroll_frame = ttk.Frame(eval_canvas)
        eval_scroll_frame.bind("<Configure>",
            lambda e: eval_canvas.configure(scrollregion=eval_canvas.bbox("all")))
        eval_canvas.create_window((0, 0), window=eval_scroll_frame, anchor="nw")
        ...
        # 마우스 휠 스크롤
        def _on_eval_mousewheel(e):
            eval_canvas.yview_scroll(int(-1 * (e.delta / 120)), "units")
        eval_canvas.bind("<Enter>", lambda e: eval_canvas.bind_all("<MouseWheel>", _on_eval_mousewheel))
        eval_canvas.bind("<Leave>", lambda e: eval_canvas.unbind_all("<MouseWheel>"))

        # inner frame 너비를 canvas에 맞추기
        def _sync_eval_width(e):
            eval_canvas.itemconfig(eval_canvas.find_all()[0], width=e.width)
        eval_canvas.bind("<Configure>", _sync_eval_width)

        parent = eval_scroll_frame

        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # ── 섹션 1: 개별 검증 프롬프트 (PASS/FAIL) ──
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        val_lf = tk.LabelFrame(parent, text=" 개별 검증 프롬프트 (PASS/FAIL 판정) ",
                               font=(FONT_FAMILY, 11, "bold"),
                               bg=CARD_BG, fg="#b91c1c", padx=8, pady=6)
        val_lf.pack(fill="x", padx=12, pady=(0, 10))

        val_info = ttk.Label(val_lf, text="(?) 이 프롬프트의 역할",
                             font=(FONT_FAMILY, 9), foreground=ACCENT, cursor="hand2")
        val_info.pack(anchor="w", padx=2, pady=(0, 4))
        ToolTip(val_info,
                "검증 프롬프트는 처리 완료 후 품질 검수에 사용됩니다.\n\n"
                "Vision API가 원본과 결과물을 비교하여\n"
                "배경 제거, 그림자, 원형 보존 3가지 항목을 PASS/FAIL 판정합니다.\n\n"
                "시스템: AI 역할 정의\n"
                "그림자 필요: 그림자가 있어야 하는 이미지의 판정 기준\n"
                "그림자 불필요: 그림자가 없어도 되는 이미지의 판정 기준\n"
                "검증 요청: 실제 검증 시 전송되는 지시문 ({image_type}, {shadow_context} 변수 사용)\n\n"
                "뷰파인더에서 '검증수정 > 개별 검증 프롬프트 수정'으로도 AI가 수정 가능합니다.")

        # 검증 시스템 프롬프트
        ttk.Label(val_lf, text="시스템 프롬프트:", font=(FONT_FAMILY, 9, "bold")).pack(anchor="w", padx=2)
        self.txt_val_system = scrolledtext.ScrolledText(val_lf, height=5,
            font=(FONT_FAMILY, 10), bg="#fef2f2", wrap="word", relief="flat",
            borderwidth=1)
        self.txt_val_system.pack(fill="x", padx=2, pady=(0, 6))

        # 그림자 필요 시 판정 기준
        ttk.Label(val_lf, text="그림자 필요 시 판정 기준:", font=(FONT_FAMILY, 9, "bold")).pack(anchor="w", padx=2)
        self.txt_val_shadow_needed = scrolledtext.ScrolledText(val_lf, height=8,
            font=(FONT_FAMILY, 10), bg="#fef2f2", wrap="word", relief="flat",
            borderwidth=1)
        self.txt_val_shadow_needed.pack(fill="x", padx=2, pady=(0, 6))

        # 그림자 불필요 시
        ttk.Label(val_lf, text="그림자 불필요 시:", font=(FONT_FAMILY, 9, "bold")).pack(anchor="w", padx=2)
        self.txt_val_shadow_not_needed = scrolledtext.ScrolledText(val_lf, height=3,
            font=(FONT_FAMILY, 10), bg="#fef2f2", wrap="word", relief="flat",
            borderwidth=1)
        self.txt_val_shadow_not_needed.pack(fill="x", padx=2, pady=(0, 6))

        # 검증 요청 템플릿
        ttk.Label(val_lf, text="검증 요청 템플릿:", font=(FONT_FAMILY, 9, "bold")).pack(anchor="w", padx=2)
        self.txt_val_user = scrolledtext.ScrolledText(val_lf, height=12,
            font=(FONT_FAMILY, 10), bg="#fef2f2", wrap="word", relief="flat",
            borderwidth=1)
        self.txt_val_user.pack(fill="x", padx=2, pady=(0, 4))

        # 구분선
        tk.Frame(parent, bg="#d1d5db", height=2).pack(fill="x", padx=12, pady=(4, 8))

        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # ── 섹션 2: 독립평가 프롬프트 (점수제) ──
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        ie_lf = tk.LabelFrame(parent, text=" 독립평가 프롬프트 (5항목 점수제 평가) ",
                               font=(FONT_FAMILY, 11, "bold"),
                               bg=CARD_BG, fg="#4338ca", padx=8, pady=6)
        ie_lf.pack(fill="x", padx=12, pady=(0, 10))

        ie_info = ttk.Label(ie_lf, text="(?) 이 프롬프트의 역할",
                             font=(FONT_FAMILY, 9), foreground=ACCENT, cursor="hand2")
        ie_info.pack(anchor="w", padx=2, pady=(0, 4))
        ToolTip(ie_info,
                "독립평가 프롬프트는 처리 완료 후 결과물의 품질을 점수로 평가합니다.\n\n"
                "검증(PASS/FAIL)과 별도로, 5가지 기준에 대해 1~10점 점수제로 평가합니다:\n"
                "  1. shadow_natural: 그림자 자연스러움\n"
                "  2. background_clean: 배경 청결도\n"
                "  3. edge_quality: 경계선 품질\n"
                "  4. product_integrity: 상품 원형 보존\n"
                "  5. commercial_quality: 상업적 활용도\n\n"
                "원본+결과 이미지를 함께 전달하여\n"
                "원본에 이미 있는 결함과 편집 과정에서 발생한 문제를 구분합니다.\n\n"
                "뷰파인더에서 '검증수정 > 독립평가 프롬프트 수정'으로도 AI가 수정 가능합니다.")

        # 독립평가 시스템 프롬프트
        ttk.Label(ie_lf, text="시스템 프롬프트:", font=(FONT_FAMILY, 9, "bold")).pack(anchor="w", padx=2)
        self.txt_ie_system = scrolledtext.ScrolledText(ie_lf, height=8,
            font=(FONT_FAMILY, 10), bg="#eef2ff", wrap="word", relief="flat",
            borderwidth=1)
        self.txt_ie_system.pack(fill="x", padx=2, pady=(0, 6))

        # 독립평가 사용자 프롬프트
        ttk.Label(ie_lf, text="평가 요청 프롬프트:", font=(FONT_FAMILY, 9, "bold")).pack(anchor="w", padx=2)
        self.txt_ie_prompt = scrolledtext.ScrolledText(ie_lf, height=16,
            font=(FONT_FAMILY, 10), bg="#eef2ff", wrap="word", relief="flat",
            borderwidth=1)
        self.txt_ie_prompt.pack(fill="x", padx=2, pady=(0, 4))
    ...
    def _load_eval_prompts(self):
        ...
        try:
            ...
            self.txt_val_system.delete("1.0", "end")
            self.txt_val_system.insert("1.0", validation.get("system", "").strip())
            self.txt_val_shadow_needed.delete("1.0", "end")
            self.txt_val_shadow_needed.insert("1.0", validation.get("shadow_needed", "").strip())
            self.txt_val_shadow_not_needed.delete("1.0", "end")
            self.txt_val_shadow_not_needed.insert("1.0", validation.get("shadow_not_needed", "").strip())
            self.txt_val_user.delete("1.0", "end")
            self.txt_val_user.insert("1.0", validation.get("user_template", "").strip())

            # 독립평가
            ind_eval = data.get("independent_evaluation", {})
            self.txt_ie_system.delete("1.0", "end")
            self.txt_ie_system.insert("1.0", ind_eval.get("system", "").strip())
            self.txt_ie_prompt.delete("1.0", "end")
            self.txt_ie_prompt.insert("1.0", ind_eval.get("prompt", "").strip())

            self.eval_prompt_status.config(text="평가 프롬프트 로드 완료", foreground=SUCCESS)
        except Exception as e:
            self.eval_prompt_status.config(text=f"로드 실패: {e}", foreground=DANGER)
    ...
    def _save_eval_prompts(self):
        ...
        try:
            ...
            data["validation"]["system"] = self.txt_val_system.get("1.0", "end").strip() + "\n"
            data["validation"]["shadow_needed"] = self.txt_val_shadow_needed.get("1.0", "end").strip() + "\n"
            data["validation"]["shadow_not_needed"] = self.txt_val_shadow_not_needed.get("1.0", "end").strip() + "\n"
            data["validation"]["user_template"] = self.txt_val_user.get("1.0", "end").strip() + "\n"

            # 독립평가
            data.setdefault("independent_evaluation", {})
            data["independent_evaluation"]["system"] = self.txt_ie_system.get("1.0", "end").strip() + "\n"
            data["independent_evaluation"]["prompt"] = self.txt_ie_prompt.get("1.0", "end").strip() + "\n"

            save_yaml(PROMPTS_PATH, data)
            self.eval_prompt_status.config(
                text=f"저장 완료 ({datetime.now().strftime('%H:%M:%S')})",
                foreground=SUCCESS)
        except Exception as e:
            self.eval_prompt_status.config(text=f"저장 실패: {e}", foreground=DANGER)

    def _reset_eval_prompts(self):
        """평가 프롬프트 탭 — 한글 기본값 복원."""
        # 개별 검증 기본값
        self.txt_val_system.delete("1.0", "end")
        self.txt_val_system.insert("1.0",
            "당신은 상품 이미지 품질 검수 전문가입니다.\n"
            "원본 이미지와 처리된 결과 이미지를 비교하여 품질을 엄격하게 검증합니다.\n"
            "애매한 경우 FAIL로 판정하세요.\n"
            "반드시 JSON만 출력하세요. 설명 텍스트 없이 순수 JSON만 응답하세요.")
        self.txt_val_shadow_needed.delete("1.0", "end")
        self.txt_val_shadow_needed.insert("1.0",
            "이 이미지는 그림자가 반드시 있어야 하는 이미지입니다.\n"
            "그림자 판정 기준:\n"
            "- 제품 하단에 자연스러운 접지 그림자(contact shadow)가 있으면 PASS\n"
            "- 그림자가 전혀 없으면 FAIL\n"
            "- 바닥 반사(reflection/mirror effect)가 있으면 FAIL (detail: '바닥 반사 효과 감지')\n"
            "- 피사체를 정면에서 바라보고 촬영된 이미지라면 그림자는 피사체 하단에만 있어야 PASS. "
            "측면이나 상단에 그림자가 있으면 FAIL (detail: '정면 촬영인데 그림자 위치 부적절')\n"
            "- 그림자 강도가 원본보다 다소 강하거나 약해도, 자연스러운 범위 내라면 PASS. 강도 차이만으로 FAIL하지 마세요.")
        self.txt_val_shadow_not_needed.delete("1.0", "end")
        self.txt_val_shadow_not_needed.insert("1.0",
            "이 이미지는 그림자가 불필요한 이미지입니다. 그림자 항목은 PASS로 판정하세요.")
        self.txt_val_user.delete("1.0", "end")
        self.txt_val_user.insert("1.0",
            "첫 번째 이미지는 원본, 두 번째 이미지는 처리 결과입니다.\n"
            "상품 유형: {image_type}\n"
            "{shadow_context}\n\n"
            "아래 3가지 항목을 검증하고 JSON으로 응답하세요:\n\n"
            "1. background: 배경이 깨끗한 흰색인가?\n"
            "2. shadow: 위의 그림자 판정 기준을 엄격히 적용하세요.\n"
            "3. integrity: 상품의 원형이 보존되었는가?\n\n"
        ...
        # 독립평가 기본값
        self.txt_ie_system.delete("1.0", "end")
        self.txt_ie_system.insert("1.0",
            "당신은 럭셔리 이커머스 플랫폼의 이미지 품질 감독관입니다.\n"
            "쇼핑몰에 게시될 상품 이미지를 비판적으로 평가합니다.\n"
            "이미지 처리 과정에서 발생한 문제점을 찾아내는 것이 목적입니다.\n"
            "고객이 이 이미지를 보고 구매를 결정한다는 관점에서 평가하세요.\n"
            "반드시 JSON만 출력하세요.\n\n"
            "★ 매우 중요: 첫 번째 이미지는 원본, 두 번째 이미지는 처리 결과입니다.\n"
            "원본 상품에 이미 존재하는 얼룩, 사용 흔적, 자국, 스크래치, 오염 등은 편집 결함이 아닙니다.\n"
            "이러한 원본 고유의 특성은 절대 감점 요인으로 삼지 마세요.\n"
            "오직 이미지 처리(배경 제거, 그림자 생성, 보정 등) 과정에서 새로 발생한 문제만 평가하세요.")
        self.txt_ie_prompt.delete("1.0", "end")
        self.txt_ie_prompt.insert("1.0",
            "첫 번째 이미지는 원본 상품 사진이고, 두 번째 이미지는 처리된 결과물입니다.\n"
            "원본과 비교하여 처리 과정에서 발생한 문제만 평가하세요.\n"
            "원본에 이미 있던 상품의 얼룩, 자국, 스크래치 등은 감점하지 마세요.\n\n"
            "아래 5가지 기준으로 엄격하게 평가하세요.\n"
            "각 항목 1~10점 (10=완벽, 1=심각한 문제).\n"
            "7점 미만이면 구체적 문제점을 반드시 기술하세요.\n\n"
            "1. shadow_natural: 그림자의 자연스러움\n"
            "2. background_clean: 배경 청결도\n"
            "3. edge_quality: 피사체 경계선 품질\n"
            "4. product_integrity: 상품 원형 보존 (★ 원본에 이미 있는 얼룩 등은 감점 금지)\n"
            "5. commercial_quality: 상업적 활용도\n\n"
            "응답 형식 (JSON만):\n"
            '{"shadow_natural": {"score": 8, "issues": []}, '
            '"background_clean": {"score": 9, "issues": []}, '
            '"edge_quality": {"score": 7, "issues": ["문제점"]}, '
            '"product_integrity": {"score": 9, "issues": []}, '
            '"commercial_quality": {"score": 8, "issues": []}, '
            '"overall_score": 8.2, '
            '"critical_issues": ["가장 심각한 문제"], '
            '"recommendation": "개선 제안"}')

        self.eval_prompt_status.config(text="한글 기본값 복원됨 (저장 필요)", foreground="#ca8a04")

    # ── 탭 3: 그림자 힌트 ──
    def _build_shadow_hints_tab(self):
        tab = self.tab_shadow_hints

        # ── 상단 안내 ──
        info_f = ttk.Frame(tab)
        info_f.pack(fill="x", padx=12, pady=(8, 2))
        ttk.Label(info_f, text="그림자 힌트 관리",
                  style="Header.TLabel").pack(side="left")
        ttk.Label(info_f,
                  text="카테고리 / 촬영방향 / 촬영유형 조합별 그림자 프롬프트 보충 지시",
                  font=(FONT_FAMILY, 9), foreground="#6b7280").pack(side="left", padx=(12, 0))

        # ── 우선순위 안내 ──
        prio_f = ttk.Frame(tab)
        prio_f.pack(fill="x", padx=12, pady=(0, 4))
        ttk.Label(prio_f,
            text="조회 우선순위:  provider/category/angle/type  >  category/angle  >  category  >  angle  >  default",
            font=(FONT_FAMILY, 8), foreground="#9ca3af").pack(anchor="w")
        ttk.Label(prio_f,
            text="main_prompt(공통 규칙) + 매칭된 hint(조건별 보충) = 최종 프롬프트  |  자동수정 시 해당 키의 hint만 변경되므로 다른 상품에 영향 없음",
            font=(FONT_FAMILY, 8), foreground="#9ca3af").pack(anchor="w")

        # ── 버튼 바 ──
        bf = ttk.Frame(tab)
        bf.pack(fill="x", padx=12, pady=(0, 6))
        ttk.Button(bf, text="  선택 항목 저장  ", command=self._on_hint_edit_save,
                   style="Accent.TButton").pack(side="left", padx=(0, 8), ipady=2)
        ttk.Button(bf, text="새 힌트 추가", command=self._on_hint_add).pack(
            side="left", padx=(0, 8), ipady=2)
        ttk.Button(bf, text="삭제", command=self._on_hint_delete).pack(
            side="left", padx=(0, 8), ipady=2)
        ttk.Button(bf, text="파일에서 다시 불러오기",
                   command=self._load_shadow_hints).pack(side="left", ipady=2)
        self.hint_status = ttk.Label(bf, text="", style="TLabel")
        self.hint_status.pack(side="left", padx=15)

        # ── 메인 영역: 좌측 트리 + 우측 편집기 ──
        pw = tk.PanedWindow(tab, orient="horizontal", bg=BG_COLOR,
                            sashwidth=5, sashrelief="raised")
        pw.pack(fill="both", expand=True, padx=12, pady=(0, 8))

        # ── 좌측: 계층형 트리 ──
        left_f = ttk.Frame(pw)
        pw.add(left_f, width=300, minsize=200)

        tree_label_f = ttk.Frame(left_f)
        tree_label_f.pack(fill="x")
        ttk.Label(tree_label_f, text="힌트 목록",
                  font=(FONT_FAMILY, 9, "bold")).pack(side="left", padx=4, pady=2)

        tree_container = ttk.Frame(left_f)
        tree_container.pack(fill="both", expand=True)

        self.hint_tree = ttk.Treeview(tree_container, columns=("preview",),
                                      show="tree headings",
                                      selectmode="browse")
        self.hint_tree.heading("#0", text="키", anchor="w")
        self.hint_tree.heading("preview", text="미리보기", anchor="w")
        self.hint_tree.column("#0", width=180, minwidth=120)
        self.hint_tree.column("preview", width=200, minwidth=100)
        self.hint_tree.pack(fill="both", expand=True, side="left")

        tree_sb = ttk.Scrollbar(tree_container, orient="vertical",
                                command=self.hint_tree.yview)
        self.hint_tree.configure(yscrollcommand=tree_sb.set)
        tree_sb.pack(side="right", fill="y")

        self.hint_tree.bind("<<TreeviewSelect>>", self._on_hint_select)

        # 트리 태그 스타일
        self.hint_tree.tag_configure("group", font=(FONT_FAMILY, 10, "bold"))
        self.hint_tree.tag_configure("category", font=(FONT_FAMILY, 9, "bold"),
                                     foreground="#7c3aed")
        self.hint_tree.tag_configure("provider", font=(FONT_FAMILY, 9, "bold"),
                                     foreground="#0891b2")
        self.hint_tree.tag_configure("leaf", font=(FONT_FAMILY, 9))

        # ── 우측: 편집기 ──
        right_f = ttk.Frame(pw)
        pw.add(right_f, minsize=300)

        # 편집 중인 키 표시
        self._hint_key_label = ttk.Label(right_f, text="(항목을 선택하세요)",
                                         font=(FONT_FAMILY, 10, "bold"),
                                         foreground=ACCENT)
        self._hint_key_label.pack(anchor="w", padx=4, pady=(4, 2))

        # ── 그림자 프롬프트 영역 ──
        shadow_lf = tk.LabelFrame(right_f, text=" 그림자 프롬프트 ",
                                   font=(FONT_FAMILY, 9, "bold"),
                                   bg=CARD_BG, fg="#7c3aed", padx=4, pady=4)
        shadow_lf.pack(fill="both", expand=True, padx=2, pady=(0, 4))

        self.txt_hint_editor = scrolledtext.ScrolledText(
            shadow_lf, height=10, font=(FONT_FAMILY, 10), bg="#f5f0ff",
            wrap="word", relief="flat", borderwidth=1)
        self.txt_hint_editor.pack(fill="both", expand=True)

        # ── 검증 기준 영역 ──
        val_lf = tk.LabelFrame(right_f, text=" 검증 기준 (카테고리별 합격/불합격 판정) ",
                                font=(FONT_FAMILY, 9, "bold"),
                                bg=CARD_BG, fg="#0891b2", padx=4, pady=4)
        val_lf.pack(fill="both", expand=True, padx=2, pady=(0, 4))

        self.txt_validation_editor = scrolledtext.ScrolledText(
            val_lf, height=8, font=(FONT_FAMILY, 10), bg="#f0f9ff",
            wrap="word", relief="flat", borderwidth=1)
        self.txt_validation_editor.pack(fill="both", expand=True)

        # 편집기 하단 도움말
        help_f = tk.LabelFrame(right_f, text=" 작성 가이드 ",
                               font=(FONT_FAMILY, 9, "bold"),
                               bg=CARD_BG, fg="#555", padx=6, pady=4)
        help_f.pack(fill="x", padx=2, pady=(0, 2))
        guide_text = (
            "• 그림자 프롬프트: 메인 프롬프트 뒤에 추가되는 카테고리별 그림자 생성 지시\n"
            "• 검증 기준: 이 카테고리의 처리 결과를 합격/불합격 판정할 때 사용하는 기준\n"
            "• 검증 기준 예: '가방 형태 완전 보존, 손잡이/지퍼 훼손 없음, 배경 순백색'\n"
            "• 비어있으면 공통 검증 프롬프트(prompts.yaml)가 사용됩니다"
        )
        ttk.Label(help_f, text=guide_text, font=(FONT_FAMILY, 8),
                  foreground="#6b7280", background=CARD_BG,
                  wraplength=500, justify="left").pack(anchor="w")

        # 초기화
        self._shadow_hints_data = {}
        self._validation_hints_data = {}
        self._hint_selected_key = None
        self._load_shadow_hints()
    ...
    def _load_prompts(self):
        try:
            ...
            self.prompt_status.config(text="프롬프트 로드 완료", foreground=SUCCESS)
        except Exception as e:
            self.prompt_status.config(text=f"로드 실패: {e}", foreground=DANGER)

    # ── 그림자 힌트 탭 메서드 ──

    def _load_shadow_hints(self):
        """shadow_hints.yaml 로드 → 계층형 트리 구성."""
        try:
            ...
            for k, v in data.items():
                if isinstance(v, dict):
                    self._shadow_hints_data[k] = str(v.get("shadow_hint", "")).strip()
                    self._validation_hints_data[k] = str(v.get("validation", "")).strip()
                else:
                    self._shadow_hints_data[k] = str(v).strip()
                    self._validation_hints_data[k] = ""
            self._rebuild_hint_tree()
            self.txt_hint_editor.delete("1.0", "end")
            self.txt_validation_editor.delete("1.0", "end")
            self._hint_selected_key = None
            self.hint_status.config(text="힌트 로드 완료", foreground=SUCCESS)
        except Exception as e:
            self.hint_status.config(text=f"힌트 로드 실패: {e}", foreground=DANGER)

    def _rebuild_hint_tree(self):
        """_shadow_hints_data → 계층형 트리뷰 재구성."""
        tree = self.hint_tree
        ...
        group_default = {}      # "default" 등 단독 키
        group_angle = {}        # 촬영방향 (top_down, side 등)
        group_category = {}     # 카테고리별 (hat, shoes 등)
        group_provider = {}     # provider별 (gemini/..., grok/...)
        ...
        for key in sorted(self._shadow_hints_data.keys()):
            ...
            if parts[0] in KNOWN_PROVIDERS:
                prov = parts[0]
                group_provider.setdefault(prov, []).append(key)
            elif key == "default":
                group_default[key] = True
            elif key in KNOWN_ANGLES:
                group_angle[key] = True
            elif key in KNOWN_TYPES and key not in KNOWN_ANGLES:
                # "package" 같은 타입 단독 키 → 촬영방향그룹에 포함
                group_angle[key] = True
            elif "/" in key:
                # category/angle 또는 category/angle/type
                cat = parts[0]
                group_category.setdefault(cat, []).append(key)
            else:
                # 단독 카테고리 키 (hat, shoes 등)
                group_category.setdefault(key, []).append(key)

        # ── 기본 ──
        if group_default:
            node_def = tree.insert("", "end", iid="__grp_default",
                                   text="기본 (default)", open=True,
                                   tags=("group",))
            for k in group_default:
                preview = self._shadow_hints_data[k][:50].replace("\n", " ")
                tree.insert(node_def, "end", iid=k, text=k,
                            values=(preview,), tags=("leaf",))

        # ── 촬영방향별 ──
        if group_angle:
            node_angle = tree.insert("", "end", iid="__grp_angle",
                                     text="촬영방향별", open=True,
                                     tags=("group",))
            for k in sorted(group_angle.keys()):
                preview = self._shadow_hints_data[k][:50].replace("\n", " ")
                tree.insert(node_angle, "end", iid=k, text=k,
                            values=(preview,), tags=("leaf",))

        # ── 카테고리별 ──
        if group_category:
            node_cat = tree.insert("", "end", iid="__grp_category",
                                   text="카테고리별", open=True,
                                   tags=("group",))
            for cat in sorted(group_category.keys()):
                keys = sorted(group_category[cat])
                # 카테고리 노드
                cat_node = tree.insert(node_cat, "end", iid=f"__cat_{cat}",
                                       text=cat, open=True,
                                       tags=("category",))
                for k in keys:
                    # 하위 키 표시: category 단독이면 "(기본)", 아니면 나머지 부분
                    parts = k.split("/")
                    display = "(기본)" if len(parts) == 1 else "/".join(parts[1:])
                    preview = self._shadow_hints_data[k][:50].replace("\n", " ")
                    tree.insert(cat_node, "end", iid=k, text=display,
                                values=(preview,), tags=("leaf",))

        # ── Provider별 ──
        if group_provider:
            node_prov = tree.insert("", "end", iid="__grp_provider",
                                    text="Provider별", open=False,
                                    tags=("group",))
            for prov in sorted(group_provider.keys()):
                keys = sorted(group_provider[prov])
                prov_node = tree.insert(node_prov, "end", iid=f"__prov_{prov}",
                                        text=prov, open=True,
                                        tags=("provider",))
                for k in keys:
                    parts = k.split("/")
                    ...
                    preview = self._shadow_hints_data[k][:50].replace("\n", " ")
                    tree.insert(prov_node, "end", iid=k, text=display,
                                values=(preview,), tags=("leaf",))

    def _save_shadow_hints(self):
        try:
            merged = {}
            for key in self._shadow_hints_data:
                shadow = self._shadow_hints_data.get(key, "")
                val = self._validation_hints_data.get(key, "")
                if val:
                    merged[key] = {"shadow_hint": shadow, "validation": val}
                else:
                    merged[key] = shadow  # 검증 기준 없으면 기존 문자열 형태 유지
            save_yaml(SHADOW_HINTS_PATH, merged)
            self.hint_status.config(text="저장 완료", foreground=SUCCESS)
        except Exception as e:
            self.hint_status.config(text=f"저장 실패: {e}", foreground=DANGER)

    def _on_hint_select(self, event=None):
        sel = self.hint_tree.selection()
        if not sel:
            return
        iid = sel[0]
        # 그룹 노드는 편집 불가
        if iid.startswith("__"):
            ...
            self.txt_validation_editor.delete("1.0", "end")
            self._hint_selected_key = None
            self._hint_key_label.config(text="(그룹 선택 — 하위 항목을 선택하세요)")
            return
        self._hint_selected_key = iid
        self._hint_key_label.config(text=f"편집 중: {iid}")
        self.txt_hint_editor.delete("1.0", "end")
        self.txt_hint_editor.insert("1.0", self._shadow_hints_data.get(iid, ""))
        self.txt_validation_editor.delete("1.0", "end")
        self.txt_validation_editor.insert("1.0", self._validation_hints_data.get(iid, ""))

    def _on_hint_edit_save(self):
        key = getattr(self, "_hint_selected_key", None)
        if not key:
            self.hint_status.config(text="저장할 힌트를 선택하세요", foreground=DANGER)
            return
        text = self.txt_hint_editor.get("1.0", "end-1c").strip()
        self._shadow_hints_data[key] = text
        val_text = self.txt_validation_editor.get("1.0", "end-1c").strip()
        self._validation_hints_data[key] = val_text
        # 트리 미리보기 갱신
        try:
            preview = text[:50].replace("\n", " ")
            self.hint_tree.item(key, values=(preview,))
        except Exception:
            pass
        self._save_shadow_hints()

    def _on_hint_add(self):
        """새 힌트 추가 — 카테고리와 촬영방향을 조합하여 키 생성."""
        dlg = tk.Toplevel(self)
        dlg.title("새 그림자 힌트 추가")
        dlg.geometry("420x320")
        dlg.resizable(False, False)
        dlg.transient(self)
        dlg.grab_set()

        ttk.Label(dlg, text="새 힌트 키 구성", font=(FONT_FAMILY, 11, "bold")).pack(
            anchor="w", padx=12, pady=(10, 6))

        # Provider (선택)
        prov_f = ttk.Frame(dlg)
        prov_f.pack(fill="x", padx=12, pady=2)
        ttk.Label(prov_f, text="Provider (선택):", width=16).pack(side="left")
        var_prov = tk.StringVar(value="(공통)")
        cb_prov = ttk.Combobox(prov_f, textvariable=var_prov, width=20,
                               values=["(공통)", "gemini_shadow", "grok_shadow"])
        cb_prov.pack(side="left", padx=4)

        # 카테고리
        cat_f = ttk.Frame(dlg)
        cat_f.pack(fill="x", padx=12, pady=2)
        ttk.Label(cat_f, text="카테고리:", width=16).pack(side="left")
        var_cat = tk.StringVar(value="(없음)")
        known_cats = ["(없음)", "bag", "shoes", "clothing", "hat", "accessory",
                      "watch", "jewelry", "wallet", "belt", "scarf", "sunglasses", "other"]
        cb_cat = ttk.Combobox(cat_f, textvariable=var_cat, width=20, values=known_cats)
        cb_cat.pack(side="left", padx=4)
        ttk.Label(cat_f, text="또는 직접 입력", font=(FONT_FAMILY, 8),
                  foreground="#9ca3af").pack(side="left", padx=4)

        # 촬영방향
        angle_f = ttk.Frame(dlg)
        angle_f.pack(fill="x", padx=12, pady=2)
        ttk.Label(angle_f, text="촬영방향:", width=16).pack(side="left")
        var_angle = tk.StringVar(value="(없음)")
        known_angles = ["(없음)", "front", "high_angle", "top_down", "side",
                        "detail", "held", "worn"]
        cb_angle = ttk.Combobox(angle_f, textvariable=var_angle, width=20,
                                values=known_angles)
        cb_angle.pack(side="left", padx=4)

        # 촬영유형
        type_f = ttk.Frame(dlg)
        type_f.pack(fill="x", padx=12, pady=2)
        ttk.Label(type_f, text="촬영유형:", width=16).pack(side="left")
        var_type = tk.StringVar(value="(없음)")
        cb_type = ttk.Combobox(type_f, textvariable=var_type, width=20,
                               values=["(없음)", "full", "detail", "worn", "package"])
        cb_type.pack(side="left", padx=4)

        # 또는 직접 키 입력
        ttk.Separator(dlg, orient="horizontal").pack(fill="x", padx=12, pady=8)
        direct_f = ttk.Frame(dlg)
        direct_f.pack(fill="x", padx=12, pady=2)
        ttk.Label(direct_f, text="또는 직접 키 입력:", width=16).pack(side="left")
        var_direct = tk.StringVar()
        ent_direct = ttk.Entry(direct_f, textvariable=var_direct, width=30)
        ent_direct.pack(side="left", padx=4)

        # 미리보기
        preview_var = tk.StringVar(value="키: (선택하세요)")
        ttk.Label(dlg, textvariable=preview_var, font=(FONT_FAMILY, 10, "bold"),
                  foreground=ACCENT).pack(anchor="w", padx=12, pady=(8, 4))

        def _update_preview(*_):
            direct = var_direct.get().strip()
            if direct:
                preview_var.set(f"키: {direct}")
                return
            parts = []
            prov = var_prov.get()
            if prov and prov != "(공통)":
            ...
            preview_var.set(f"키: {key}")

        for v in (var_prov, var_cat, var_angle, var_type, var_direct):
        ...
        def _on_ok():
            ...
            if direct:
                ...
                key = "/".join(parts)
            if not key:
                messagebox.showwarning("입력 오류", "키를 구성하세요.", parent=dlg)
                return
            if key in self._shadow_hints_data:
                messagebox.showwarning("중복",
                    f"이미 존재하는 키입니다: {key}", parent=dlg)
                return
            self._shadow_hints_data[key] = ""
            self._validation_hints_data[key] = ""
            self._rebuild_hint_tree()
            # 새 항목 선택
            try:
                self.hint_tree.selection_set(key)
                self.hint_tree.see(key)
                self._hint_selected_key = key
                self._hint_key_label.config(text=f"편집 중: {key}")
                self.txt_hint_editor.delete("1.0", "end")
            except Exception:
                pass
            self.hint_status.config(
                text=f"'{key}' 추가됨 — 내용 입력 후 [저장]",
                foreground=SUCCESS)
            dlg.destroy()

        ttk.Button(btn_f, text="추가", command=_on_ok,
                   style="Accent.TButton").pack(side="left", padx=(0, 8))
        ttk.Button(btn_f, text="취소", command=dlg.destroy).pack(side="left")

    def _on_hint_delete(self):
        key = getattr(self, "_hint_selected_key", None)
        if not key:
            self.hint_status.config(text="삭제할 힌트를 선택하세요", foreground=DANGER)
            return
        if key == "default":
            messagebox.showwarning("삭제 불가", "기본(default) 힌트는 삭제할 수 없습니다.",
                                   parent=self)
            return
        if not messagebox.askyesno("삭제 확인", f"'{key}' 힌트를 삭제하시겠습니까?"):
            return
        self._shadow_hints_data.pop(key, None)
        self._validation_hints_data.pop(key, None)
        self._hint_selected_key = None
        self._hint_key_label.config(text="(선택 없음)")
        self.txt_hint_editor.delete("1.0", "end")
        self.txt_validation_editor.delete("1.0", "end")
        self._rebuild_hint_tree()
        self._save_shadow_hints()
        self.hint_status.config(text=f"'{key}' 삭제됨", foreground=SUCCESS)

    def _save_workers_setting(self):
        """동시 처리 수 변경 시 즉시 settings.yaml에 저장."""
        try:
            data = load_yaml(SETTINGS_PATH)
            data["concurrent_workers"] = self.var_workers.get()
    ...
    def _save_prompts(self):
        try:
            ...
            self.prompt_status.config(text=f"저장 실패: {e}", foreground=DANGER)
    ...
    def _reset_prompts(self):
        ...
        self.txt_user.insert("1.0",
            ...
            '  "needs_shadow": <boolean. 이커머스 표시용 바닥 그림자가 필요한지.\n'
            '    - 바닥에 놓인 가방/신발/소품 등 정면/측면 촬영 → true\n'
            '    - 모델 착용샷에서 발(신발)이 바닥에 닿아 보이는 경우 → true\n'
            '    - 마네킹/행거 의류(바닥이 안 보임), 플랫레이, 디테일컷 → false\n'
            '    - 탑다운(위에서 내려다본) 촬영 → false>,\n'
        ...
        self.txt_ie_system.insert("1.0",
            "당신은 럭셔리 이커머스 플랫폼의 이미지 품질 감독관입니다.\n"
            "쇼핑몰에 게시될 상품 이미지를 비판적으로 평가합니다.\n"
            "합격/불합격을 판정하는 것이 아니라, 이미지 처리 과정에서 발생한 문제점을 찾아내는 것이 목적입니다.\n"
            "고객이 이 이미지를 보고 구매를 결정한다는 관점에서 평가하세요.\n"
            "반드시 JSON만 출력하세요.\n\n"
            "★ 매우 중요: 첫 번째 이미지는 원본, 두 번째 이미지는 처리 결과입니다.\n"
            "원본 상품에 이미 존재하는 얼룩, 사용 흔적, 자국, 스크래치, 오염 등은 편집 결함이 아닙니다.\n"
        ...
        self.prompt_status.config(text="한글 기본값 복원됨 (저장 필요)", foreground="#ca8a04")
    ...
    def _set_progress(self, current, total):
        ...
        def _do():
            ...
            self.status_bar.config(text=f"처리 중... {current}/{total}")
        self.after(0, _do)
    ...
    def _set_running(self, running):
        ...
        self.btn_run_single.config(state=state)
        self.btn_run_batch.config(state=state)
    ...
    def _browse_single_file(self):
        """이미지 파일 선택 대화상자 (다중 선택 가능). 기존 입력 경로를 초기 디렉토리로 사용."""
        filetypes = [
    ...
    def _run(self, mode):
        if mode == "single":
            # 다중 파일 선택 대화상자 표시
            selected = self._browse_single_file()
            if not selected:
                return
            # 존재하지 않는 파일 필터링
            valid_files = [f for f in selected if Path(f).is_file()]
            if not valid_files:
                messagebox.showerror("오류", "선택한 파일이 존재하지 않습니다.")
                return
            self._selected_files = valid_files
            input_path = valid_files[0]
        else:
        ...
        if not self.var_skip_analysis.get():
            ...
            elif vision_prov == "grok":
                xai_key = self.var_xai_key.get().strip() or os.environ.get("XAI_API_KEY", "")
                if not xai_key:
                    messagebox.showwarning("API 키 필요",
                        "Grok Vision 분석을 위해 xAI API 키가 필요합니다.\n"
                        "[설정] 탭에서 XAI_API_KEY를 입력하거나,\n"
                        "[AI분석 생략] 옵션을 체크하세요.")
                    return
        ...
        # Grok 그림자 API 키 검증
        shadow_prov = self.var_shadow_provider.get()
        if shadow_prov == "grok_shadow":
            xai_key = self.var_xai_key.get().strip() or os.environ.get("XAI_API_KEY", "")
            if not xai_key:
                messagebox.showwarning("API 키 필요",
                    "Grok 그림자 생성을 위해 xAI API 키가 필요합니다.\n"
                    "[설정] 탭에서 XAI_API_KEY를 입력하세요.")
                return
            os.environ["XAI_API_KEY"] = xai_key
    ...
    def _run_worker(self, mode):
        try:
            ...
            self._viewfinder_pairs = []
            self._vf_file_stages = {}
            self.after(0, lambda: self.btn_viewfinder.config(state="normal"))
            ...
            _temp_stages = Path(output_dir) / "_temp_stages"
            if _temp_stages.exists():
                _shutil.rmtree(_temp_stages, ignore_errors=True)
            skip_analysis = self.var_skip_analysis.get()
            skip_bg = self.var_skip_bg.get()
            pre_cropped = self.var_pre_cropped.get()
            ...
            if mode == "analyze":
                ...
                total = len(files)
                num_workers = self.var_workers.get()
                self._set_progress(0, total)

                if total == 1:
                    target = files[0]
                    vf_idx = self._vf_register_file(target)
                    _vf_log = self._vf_make_log(Path(target).name)
                    if self.var_auto_refine.get():
                        ...
                        _stage_cb = self._vf_make_stage_cb(Path(target).name, output_dir)
                        result = pipeline.process_single(
                            image_path=target, category=category, output_dir=output_dir,
                            skip_analysis=skip_analysis, skip_photoroom=skip_bg,
                            pre_cropped=pre_cropped,
                            on_log=_vf_log,
                            on_stage_image=_stage_cb)
                        self._log_result(result)
                    self._vf_complete_file(vf_idx, result)
                    self._set_progress(1, 1)

                else:
                    self._log(f"━━━ 다중 파일 처리: {total}개 파일, 동시 {num_workers}개 ━━━")
                    ...
                    def _process_one(file_path):
                        nonlocal success_count, fail_count, total_output_files, completed
                        fname = Path(file_path).name
                        with lock:
                            vf_idx = self._vf_register_file(file_path)
                        try:
                            if not self._processing:
                                return
                            _vf_log = self._vf_make_log(
                                fname,
                                base_log=lambda msg, level="info": self._log(f"[{fname}] {msg}", level))
                            from src.pipeline import ImageEditPipeline
                            thread_pipeline = ImageEditPipeline(config_dir=str(CONFIG_DIR))
                            _stage_cb = self._vf_make_stage_cb(fname, output_dir)

                            result = thread_pipeline.process_single(
                                image_path=file_path, category=category,
                                output_dir=output_dir,
                                skip_analysis=skip_analysis, skip_photoroom=skip_bg,
                                pre_cropped=pre_cropped,
                                on_log=_vf_log,
                            ...
                            with lock:
                                ...
                                if out_files:
                                    success_count += 1
                                    total_output_files += len(out_files)
                                    self._log(f"[{fname}] \u2713 완료 ({completed}/{total})", "success")
                                else:
                                    fail_count += 1
                                    err = result.get("error", "출력 파일 없음")
                                    self._log(f"[{fname}] \u2717 실패: {err} ({completed}/{total})", "error")
                            ...
                            with lock:
                                completed += 1
                                fail_count += 1
                                self._log(f"[{fname}] \u2717 오류: {e} ({completed}/{total})", "error")
                                self._set_progress(completed, total)

                    if num_workers <= 1:
                        for f in files:
                            if not self._processing:
                                self._log("사용자에 의해 중지됨.", "warning")
                                break
                            _process_one(f)
                    else:
                        from concurrent.futures import ThreadPoolExecutor, as_completed
                        with ThreadPoolExecutor(max_workers=num_workers) as executor:
                            ...
                            for future in as_completed(futures):
                                if not self._processing:
                                    self._log("사용자에 의해 중지됨. 진행 중인 작업 완료 대기...", "warning")
                                    executor.shutdown(wait=False, cancel_futures=True)
                                    break
                                try:
                                    future.result()
                    ...
                    self._log(f"━━━ 처리 완료: 성공 {success_count}/{total}, "
                              f"실패 {fail_count}, 총 {total_output_files}개 파일 생성 ━━━", "success")
                ...
                if num_workers <= 1:
                    ...
                    for i, r in enumerate(results):
                        inp = r.get("path", "")
                        out_files = r.get("files", [])
                        ok = r.get("success", False)
                        if i < len(self._viewfinder_pairs):
                            self._viewfinder_pairs[i]["output_files"] = out_files
                            self._viewfinder_pairs[i]["success"] = ok
                            self._viewfinder_pairs[i]["status"] = "done" if ok else "fail"
                            ind_eval = r.get("independent_eval")
                            if ind_eval:
                                self._viewfinder_pairs[i]["independent_eval"] = ind_eval
                        fname = Path(inp).name
                        if fname in self._vf_file_stages:
                            self._vf_file_stages[fname]["status"] = "done" if ok else "fail"
                            if ok:
                                for s in self._VF_STAGES:
                                    self._vf_file_stages[fname]["stages"][s] = "done"
                else:
                    self._log(f"\u2501\u2501\u2501 병렬 배치 처리: {total}개 파일, 동시 {num_workers}개 \u2501\u2501\u2501")
                    results = []
                    completed = 0
                    lock = threading.Lock()
                    self._set_progress(0, total)

                    def _batch_one(file_path):
                        nonlocal completed
                        fname = Path(file_path).name
                        with lock:
                            vf_idx = self._vf_register_file(file_path)
                        try:
                            ...
                            if out_files:
                                result["success"] = True
                                self._log(f"[{fname}] \u2713 완료", "success")
                            else:
                                result["success"] = False
                                self._log(f"[{fname}] \u2717 실패: {result.get('error', '출력 파일 없음')}", "error")
                            with lock:
                                self._vf_complete_file(vf_idx, result)
                            return result
                        except Exception as e:
                            with lock:
                                completed += 1
                                self._set_progress(completed, total)
                            self._log(f"[{fname}] \u2717 오류: {e}", "error")
                            return {"success": False, "error": str(e)}

                    from concurrent.futures import ThreadPoolExecutor, as_completed
                    with ThreadPoolExecutor(max_workers=num_workers) as executor:
                        futures = {executor.submit(_batch_one, f): f for f in files}
                        for future in as_completed(futures):
                            if not self._processing:
                                executor.shutdown(wait=False, cancel_futures=True)
                                break
            ...
            self.after(0, lambda: self.status_bar.config(text="완료"))
            pass  # 뷰파인더 버튼은 처리 시작 시 이미 활성화됨
    ...
    def _open_deliberation_window(self):
        ...
        c.tag_config("grok_name", foreground="#a855f7", font=(FONT_FAMILY, 10, "bold"))
        ...
        c.tag_config("grok_text", foreground="#c084fc", lmargin1=20, lmargin2=20)
        # 프로바이더별 배경 강조 (미묘한 차이)
        ...
        c.tag_config("grok_bg", background="#1e1028")
        # 단계 구분
    ...
    def _on_deliberation_inner(self, chat, phase, provider, data):
        ...
        names = {"claude": "Claude", "chatgpt": "ChatGPT",
                 "gemini": "Gemini", "grok": "Grok", "mc": "사회자"}
        ts = datetime.now().strftime("%H:%M:%S")
        ...
        if phase == "phase" and isinstance(data, dict):
            ...
            for i, lbl in self._phase_labels.items():
                if i < p:
                    lbl.config(fg="#a6e3a1", bg="#181825")
                elif i == p:
                    lbl.config(fg="#11111b", bg="#f5c2e7")
                else:
                    lbl.config(fg="#585b70", bg="#181825")
            self._delib_phase_label.config(text=f"{p}단계: {title}")
            chat.insert("end", f"\n{'═' * 55}\n", "phase_line")
        ...
        if isinstance(data, dict) and data.get("error"):
            ...
            err_dots = {"claude": "🟢", "chatgpt": "🔵", "gemini": "🟠", "grok": "🟣"}
            chat.insert("end", "\n  ─────────────────────\n", "speaker_sep")
        ...
        dots = {"claude": "🟢", "chatgpt": "🔵", "gemini": "🟠", "grok": "🟣"}
...
if __name__ == "__main__":
    ...
    def _thread_exception_handler(args):
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with open(_crash_log, "a", encoding="utf-8") as f:
            f.write(f"\n{'='*60}\n[{timestamp}] Thread Exception ({args.thread})\n{msg}\n")
